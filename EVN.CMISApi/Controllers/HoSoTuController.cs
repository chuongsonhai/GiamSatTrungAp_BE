...
{
    [RoutePrefix("api/hosotu")]
    public class HoSoTuController : ApiController
    {
        [HttpGet]
        ...
        public IHttpActionResult Filter(string maDVi, string keyword)
        {
            ...
            try
            {
                ...
                sql.AppendLine("select * from DIEMDO.DD_HSO_TU WHERE MA_DVI_SD=:MA_DVI_SD AND SO_TU=:SO_TU");
                cmd.CommandText = sql.ToString();
                cmd.Parameters.Add(new OracleParameter("MA_DVI_SD", maDVi));
                cmd.Parameters.Add(new OracleParameter("SO_TU", keyword));
                ...
                if (dr.HasRows)
                {
                    IList<HoSoTU> result = new List<HoSoTU>();
                    while (dr.Read())
                    {
                        HoSoTU data = new HoSoTU();
                        data.MA_TI = dr["MA_TU"].ToString();
                        data.SO_TI = dr["SO_TU"].ToString();
