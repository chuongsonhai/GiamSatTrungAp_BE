...
{
    [RoutePrefix("api/baocaottdn")]
    public class BaoCaoTTDNController : ApiController
    {
        private ILog log = LogManager.GetLogger(typeof(BaoCaoTTDNController));
        ...
        public IHttpActionResult Export(YeuCauFilter request)
        {
            try
            {
                ...
                var list = service.GetListBaoCaoTTDN(request.maDViQLy, request.keyword, request.khachhang, request.status, fromDate, toDate, 0, 0, out total);


                string fileTemplate = AppDomain.CurrentDomain.BaseDirectory + "Templates/ChiTietTTDN.xlsx";
                ...
                using (ExcelPackage package = new ExcelPackage(fileTemp, true))
                {
                    ...
                    foreach (var item in list)
                    {
                        ...
                        ws.Cells[row, colval].Value = item.ThoiGianChuyenHSSangKS;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;
                        ...
                        ws.Cells[row, colval].Value = item.ThoiGianDuKienKS;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;

                        ws.Cells[row, colval].Value = item.TroNgaiKS;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
                        colval++;

                        ws.Cells[row, colval].Value = item.ThoiGianKSThucTe;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;

                        ws.Cells[row, colval].Value = item.SoNgayThucHienKS;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;

                        ws.Cells[row, colval].Value = item.ThoiGianLapVaDuyetTTDN;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;

                        ws.Cells[row, colval].Value = item.SoNgayLapBBTTDN;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;

                        ws.Cells[row, colval].Value = item.ThoiGianChuyenBBTTDNDenKH;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;
                   
                        ws.Cells[row, colval].Value = item.SoNgayLapTTDN;
                        ws.Cells[row, colval].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
                        colval++;
