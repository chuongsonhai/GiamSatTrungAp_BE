        ...
        public IHttpActionResult Post([FromBody] BienBanKSModel model)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(ketquaks.MA_TNGAI))
                {
                    ...
                    canhbao.NOIDUNG = "Loại cảnh báo 10 - lần " + canhbao.LOAI_SOLANGUI + " <br>KH: " + item.KHDaiDien + ", SĐT: " + item.KHDienThoai + ", ĐC: " + yeucau.DiaChiCoQuan + ", MaYC: " + canhbao.MA_YC + ", ngày tiếp nhận: " + item.NgayLap +  " ĐV: " + item.MaDViQLy + " Yêu cầu kiểm tra điều kiện đóng điện điểm đấu nối và nghiệm thu bị từ chối tiếp nhận, đơn vị hãy tiếp nhận yêu cầu lập thỏa thuận đấu nối trên hệ thống Ứng dụng cấp điện mới trực tuyến và giám sát các chỉ số tiếp cận điện năng.";
