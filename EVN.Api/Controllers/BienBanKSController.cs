        ...
        public IHttpActionResult Post([FromBody] BienBanKSModel model)
        {
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(ketquaks.MA_TNGAI))
                {
                    ...
                    canhbao.NOIDUNG = "Cảnh báo lần…(" + canhbao.LOAI_SOLANGUI + ") <br>Khách hàng" + item.KHDaiDien + ", số điện thoại" + item.KHDienThoai + ", địa chỉ:" + yeucau.DiaChiCoQuan + ", mã yêu cầu" + canhbao.MA_YC + ", ngày tiếp nhận:" + item.NgayLap + "Đơn vị tiếp nhận hồ sơ:" + item.MaDViQLy + "Yêu cầu lập thỏa thuận đấu nối bị từ chối tiếp nhận, đơn vị hãy tiếp nhận yêu cầu lập thỏa thuận đấu nối trên hệ thống Ứng dụng cấp điện mới trực tuyến và giám sát các chỉ số tiếp cận điện năng.";
